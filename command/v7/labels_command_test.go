...
var _ = Describe("labels command", func() {
	var (
		...
		fakeOrgActor       *v7fakes.FakeOrgActor
	...
	BeforeEach(func() {
		...
		fakeOrgActor = new(v7fakes.FakeOrgActor)
	...
	Describe("listing labels", func() {
		...
		Describe("for orgs", func() {
			BeforeEach(func() {
				...
				fakeOrgActor.GetOrganizationByNameReturns(
			...
			FIt("retrieves the labels associated with the organization", func() {
				Expect(fakeOrgActor.GetOrganizationByNameCallCount()).To(Equal(1))
				orgName := fakeOrgActor.GetOrganizationByNameArgsForCall(0)
